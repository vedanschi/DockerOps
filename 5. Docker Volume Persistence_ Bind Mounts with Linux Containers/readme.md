...
You executed:
```sh
docker run -dit --name alpine_with_bind_mount -v C:\Users\vedanschi\docker_data:/data alpine:latest sh
```
#### 🔍 What Happened?
...
- The `-v` flag mounted the local directory `C:\Users\vedanschi\docker_data` to `/data` inside the container.
...
```sh
docker exec -it alpine_with_bind_mount sh -c "echo 'Hello, Vedanshi!' > /data/testfile.txt"
```
#### 🔍 What Happened?
...
- Since `/data` is a bind-mounted directory, the file was actually stored in `C:\Users\vedanschi\docker_data` on the host.
...
```sh
docker run -dit --name new_alpine -v C:\Users\vedanschi\docker_data:/data alpine sh
```
#### 🔍 What Happened?
- A new container named **new_alpine** was created.
- The same bind-mounted directory (`C:\Users\vedanschi\docker_data`) was mounted to `/data`.
